...
define( 'MANTILOAD_VERSION', '1.7.1' );
...
final class MantiLoad {
	...
	public function intercept_price_filter_query( $query ) {
		...
		if ( strpos( $query, 'wp_wc_product_meta_lookup' ) !== false &&
			...
			$search_term = '';
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Reading search query parameter ($_GET[s]), standard WordPress functionality
			if ( isset( $_GET['s'] ) ) {
				// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Reading search query parameter, sanitized before use
	...
	public function fix_woocommerce_chosen_attributes_early() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Frontend WooCommerce filter processing, all inputs validated
			// Debug: Check $_GET
...
// error_log( 'MantiLoad: Found filter keys in $_GET: ' . implode( ', ', $filter_keys ) );
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Processing WooCommerce filter parameters (filter_*), validated before use
...
// error_log( 'MantiLoad: term_slugs found = ' . print_r( $term_slugs, true ) );
					...
					if ( ! empty( $term_slugs ) ) {
						...
						$query_type = '';
						if ( ! empty( $_GET[ $query_type_key ] ) ) {
					// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Reading WooCommerce filter query type parameter, validated against whitelist
							$raw_query_type = sanitize_text_field( wp_unslash( $_GET[ $query_type_key ] ) );
						// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Reading WooCommerce filter query type, sanitized and validated
							$query_type = in_array( $raw_query_type, array( 'and', 'or' ), true ) ? $raw_query_type : '';
						}
...
// error_log( 'MantiLoad: Stack trace: ' . $e->getTraceAsString() );
	...
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Processing WooCommerce filter parameters for display, validated before use
	...
	private function set_default_options() {
		...
		// Example: wp_ → mantiload_wp_index, wp_uyqsre_ → mantiload_wp_uyqsre_index
